...
void PointCloudBuffer::add_color_rgba8(const std::uint8_t* colors, int num_points) {
  add_buffer("vert_color", 4, GL_UNSIGNED_BYTE, GL_TRUE, colors, sizeof(std::uint8_t) * 4, num_points);
}
...
void PointCloudBuffer::add_buffer(const std::string& attribute_name, int dim, const float* data, int stride, int num_points) {
  add_buffer(attribute_name, dim, GL_FLOAT, GL_FALSE, data, stride, num_points);
}

void PointCloudBuffer::add_buffer(
  ...
  aux_buffers.push_back(AuxBufferData{attribute_name, dim, stride, type, normalized, buffer_id});
}
...
void PointCloudBuffer::bind(glk::GLSLShader& shader) const {
  ...
  for (const auto& aux : aux_buffers) {
    ...
    glVertexAttribPointer(attrib_loc, aux.dim, aux.type, aux.normalized, aux.stride, 0);
  }
}
